...
t_type* t_type_of<xemmaix::nata::curses::t_token>::f_derive(t_object* a_this)
{
	return new t_derived<t_type_of>(t_scoped(v_module), a_this);
}

void t_type_of<xemmaix::nata::curses::t_token>::f_finalize(t_object* a_this)
{
	delete &f_as<xemmaix::nata::curses::t_token&>(a_this);
}

t_scoped t_type_of<xemmaix::nata::curses::t_token>::f_construct(t_object* a_class, t_stacked* a_stack, size_t a_n)
{
	return t_construct<attr_t>::t_bind<xemmaix::nata::curses::t_token>::f_do(a_class, a_stack, a_n);
}
