...
void t_type_of<xemmaix::nata::tree_sitter::t_query>::f_define(t_library* a_library)
{
	...
	t_define{a_library}
		(L"captures"sv, t_member<t_pvalue(t_query::*)() const, &t_query::f_captures>())
...
{
	return t_construct_with<t_pvalue(*)(t_type*, const xemmaix::nata::tree_sitter::t_language&, std::wstring_view), xemmaix::nata::tree_sitter::t_query::f_construct>::t_bind<xemmaix::nata::tree_sitter::t_query>::f_do(this, a_stack, a_n);
}
...
void t_type_of<xemmaix::nata::tree_sitter::t_parser>::f_define(t_library* a_library)
{
	...
	t_define{a_library}
		(L"parsed"sv, t_member<bool(t_parser::*)() const, &t_parser::f_parsed>())
		(L"next"sv, t_member<t_pvalue(t_parser::*)(), &t_parser::f_next>())
...
{
	return t_construct_with<t_pvalue(*)(t_type*, xemmaix::nata::t_text&, xemmaix::nata::tree_sitter::t_query&), xemmaix::nata::tree_sitter::t_parser::f_construct>::t_bind<xemmaix::nata::tree_sitter::t_parser>::f_do(this, a_stack, a_n);
}
...
t_pvalue t_query::f_construct(t_type* a_class, const t_language& a_language, std::wstring_view a_source)
{
	std::wstring_convert<std::codecvt_utf8<wchar_t>> convert;
	auto source = convert.to_bytes(a_source.data(), a_source.data() + a_source.size());
	uint32_t error_offset;
	TSQueryError error_type;
	if (auto query = ts_query_new(a_language.v_language, source.data(), source.size(), &error_offset, &error_type)) return a_class->f_new<t_query>(a_language, query);
...
t_pvalue t_query::f_captures() const
{
	return t_tuple::f_instantiate(ts_query_capture_count(v_query), [&](auto& tuple)
	{
		std::wstring_convert<std::codecvt_utf8<wchar_t>> convert;
...
t_pvalue t_parser::f_next()
{
	...
	{
		return v_p.f_base().f_at(p).f_index().v_character;
	};
